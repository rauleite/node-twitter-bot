...
stream.on('tweet', function(tweet) {
    ...
    if(
        ...
        urlNotTwitterPattern.test(text) &&
        // Tem que ter link dentro da propriedade urls - se não tiver o link da
        // propriedade 'text' é midia
        ...
        if (words.lightWhitelistPattern.test(text) && isLucky(3/4)) {
            ...
            console.log('***** is in blacklist?', user_blacklist.test(tweet.user.screen_name)); 
        ...
        // if(tweet.entities && tweet.entities.urls && tweet.entities.urls.length !== 0) {
            ...
            var validUrl = _.filter(urlsCurrent, function(o){
                ...
                console.log('********** o.expanded_url.match(urlNotTwitterPattern)', o.expanded_url.match(urlNotTwitterPattern));
                return o.expanded_url.match(urlNotTwitterPattern); 
            });

            // console.log('******* validUrl', validUrl);
            console.log('******* text', text);
            if(validUrl.length === 0) {
                console.log('****** URL Inválida urlNotTwitterPattern', validUrl);       
                return ;
            }
        // }
        ...
        if(words.htmlPattern.test(text)) {
            ...
            if (!tweet.retweeted) {
                ...
                // scheduleFuture(retweetTweet, tweet, 2);
                scheduleFuture(retweetTweet, tweet, 1);
                // favorite(tweet);
                // follow(tweet);
            }
        }
